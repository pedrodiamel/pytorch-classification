...
class NeuralNetClassifier(NeuralNetAbstract):
    ...
    def __init__(self,
        ...
        super(NeuralNetClassifier, self).__init__(patchproject,
                                                  nameproject, no_cuda, parallel, seed, print_freq, gpu)
    ...
    def create(self,
        ...
        self.cnf = nloss.ConfusionMeter(
            self.num_output_channels, normalized=True)
        ...
        self.logger_train = Logger(
            'Trn', ['loss'], self.metrics_name, self.plotter)
        self.logger_val = Logger(
            'Val', ['loss'], self.metrics_name, self.plotter)
    ...
    def training(self, data_loader, epoch=0):
        ...
        for i, sample in enumerate(data_loader):
            ...
            self.logger_train.update(
                {'loss': loss.item()},
                dict(zip(self.metrics_name, [pred[p].item()
            ...
            if i % self.print_freq == 0:
                self.logger_train.logger(
                    epoch, epoch + float(i+1)/len(data_loader), i, len(data_loader), batch_time,)
    ...
    def representation(self, data_loader):
        ...
        for i, sample in enumerate(data_loader):
            ...
            print('Representation: |{:06d}/{:06d}||{batch_time.val:.3f} ({batch_time.avg:.3f})|'.format(
    ...
    def _create_model(self, arch, num_output_channels, num_input_channels, pretrained):
        ...
        kw = {'num_classes': num_output_channels,
              'num_channels': num_input_channels, 'pretrained': pretrained}
