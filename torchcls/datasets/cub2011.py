...
class CUB2011(data.Dataset):
    ...
    def __init__(
        ...
        ids, train_index = np.loadtxt(
            os.path.join(self.root, self.base_folder, "{}.txt".format(self.train_test_split)), unpack=True
        )
    ...
    def _check_integrity(self):
        ...
        # TODO February 17, 2023: Check integrity
...
class CUB2011MetricLearning(CUB2011):
    ...
    def __init__(
        ...
        super(CUB2011MetricLearning, self).__init__(
            root, True, transform=transform, target_transform=target_transform, download=download
        )

        if train:
            classes = self.classes[: self.num_training_classes]
        else:
            classes = self.classes[self.num_training_classes : (self.num_training_classes + self.num_training_classes)]
