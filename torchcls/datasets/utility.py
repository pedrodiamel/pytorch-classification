...
def download_images(pack):
    ...
    try:
        ...
        print("Warning: Could not download image {} from {}. {}".format(key, url, e))
        return

    try:
        pil_image = Image.open(BytesIO(image_data))
    except ValueError as e:
        print("Warning: Failed to parse image {} {}. {}".format(key, url, e))
        return
    try:
        pil_image_rgb = pil_image.convert("RGB")
    except ValueError as e:
        print("Warning: Failed to convert image {} to RGB. {}".format(key, e))
        return
    try:
        pil_image_rgb.save(filename, format="JPEG", quality=90)
    except ValueError as e:
        print("Warning: Failed to save image {}. {}".format(filename, e))
        return
